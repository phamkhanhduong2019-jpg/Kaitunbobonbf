...
Dim.BackgroundTransparency = 1   -- sẽ fade in
Dim.BorderSizePixel        = 0
Dim.ZIndex                 = 1

-- Container giữa màn hình — KHÔNG có box, chỉ text
...
-- ListLayout để các dòng tự xếp thẳng đứng, căn giữa
...
local function MkLabel(txt, sz, col, bold, order)
    ...
    lb.TextTransparency      = 1     -- bắt đầu ẩn, fade in sau
...
local function MkDiv(order)
    ...
    f.LayoutOrder        = order
    f.ZIndex             = 3
    -- Con là ListLayout nên frame này sẽ được căn giữa tự động
    local inner = Instance.new("UIListLayout", f)
    inner.HorizontalAlignment = Enum.HorizontalAlignment.Center
    return f
end

-- Dòng Beli | Frag (split 2 cột)
...
-- ── Build layout (giống Vxeze Hub) ───────────────────────────────
--   BobonHub          ← title lớn, bold, xanh nhạt
--   Kaitun Blox Fruit ← sub nhỏ hơn, trắng nhạt
--   ─────────────────
--   Status: ...       ← xanh lá, bold
--   Time: 00:00:00    ← trắng nhạt
--   ─────────────────
--   Beli: 0  │  Frag: 0
--   Kills: 0
...
--  Nguyên lý:
--    1. Kill char
--    2. CharacterAdded → spam set CFrame liên tục 4 giây
--       (trong lúc đó physics chưa kịp kéo xuống biển)
--    3. Noclip vẫn chạy song song nên char không bị block
-- ══════════════════════════════════════════════════════════════════
local SPAM_SECS   = 4.0   -- giây spam sau respawn
local SPAM_TICK   = 0.04  -- interval spam (25 lần/giây)
local NEAR_DIST   = 400   -- dưới ngưỡng này dùng CFrame thẳng, khỏi respawn
...
local function RespawnTravelTo(targetCF)
    ...
    conn = LP.CharacterAdded:Connect(function(newChar)
        ...
        task.spawn(function()
            ...
            -- Spam set CFrame trong SPAM_SECS giây
...
-- Smart travel: gần thì CFrame thẳng, xa thì respawn
...
-- Kill counter — Humanoid.Died event (chính xác nhất)
...
local function AutoSecondSea()
    ...
    while kills < 50 and os.time() < timeout do
        ...
        if mob then
            ...
            Attack()
            if mob.Humanoid.Health <= 0 then kills += 1 end
        else
...
-- ══════════════════════════════════════════════════════════════════
--    LOOP 1 — QUEST LOOP (0.3s tick, nhận quest + set target)
-- ══════════════════════════════════════════════════════════════════
task.spawn(function()
        ...
        pcall(function()
            ...
            if not HasQuest() then
                ...
                if now - _G.State.LastQuest < 3 then
                    _G.BobonStatus = "Chờ nhận quest..."
                    return
                end

                _G.BobonStatus = "Nhận quest: " .. qd.M
                Travel(qd.QC)
                task.wait(0.8)
                pcall(function()
                ...
                -- Tele ngay lên đầu mob sau nhận quest
                local mob = FindMob(qd.M)
                if mob then
                    Travel(CFrame.new(mob.HumanoidRootPart.Position + Vector3.new(0,_G.Settings.FarmHeight,0)))
                    _G.State.CurrentTarget = mob
                else
                    Travel(CFrame.new(qd.MC.Position + Vector3.new(0,_G.Settings.FarmHeight,0)))
                end
                return
            end

            -- Đã có quest — tìm mob gần nhất
            local mob = FindMob(qd.M)
            if mob then
                _G.State.CurrentTarget = mob
                _G.BobonStatus = "Farming: " .. qd.M .. " | Kills: " .. _G.State.KillCount
                -- Bám theo mob liên tục
                local mhrp = mob:FindFirstChild("HumanoidRootPart")
                if mhrp then
                    Travel(CFrame.new(mhrp.Position + Vector3.new(0,_G.Settings.FarmHeight,0)))
                end
            else
                _G.State.CurrentTarget = nil
...
-- ══════════════════════════════════════════════════════════════════
--    LOOP 2 — ATTACK LOOP (Heartbeat, đánh liên tục không ngắt)
-- ══════════════════════════════════════════════════════════════════
local lastAtk = 0
RunService.Heartbeat:Connect(function()
    if tick() - lastAtk < 0.09 then return end
    lastAtk = tick()
    pcall(function()
        if _G.State.IsTraveling then return end
        ...
        if not mh or not mhr or mh.Health <= 0 then
            _G.State.CurrentTarget = nil
            return
        end
        ...
        -- Bám sát trên đầu mob, quay mặt về phía mob
        hrp.CFrame = CFrame.new(
            mhr.Position + Vector3.new(0, _G.Settings.FarmHeight, 0),
            mhr.Position
        )
...
_G.BobonStatus = "BobonHub v13 Ready!"
print("[BobonHub v13.0] ✓ Loaded | Respawn TP | Vxeze UI | Full Farm")
